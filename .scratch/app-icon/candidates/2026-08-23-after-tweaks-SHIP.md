# Ship candidate `2026-08-23-after-tweaks`
...
- SVG `.scratch/app-icon/candidates/2026-08-23-after-tweaks.svg` — **full 1024 plate**, glyph already on the 824 keyline
- PNG `.scratch/app-icon/candidates/2026-08-23-after-tweaks.png`
- Template (menu bar) `.scratch/app-icon/candidates/2026-08-23-after-tweaks-template.svg` + `-template.png` — black glyph, alpha holes, no plate
- State `.scratch/app-icon/candidates/2026-08-23-after-tweaks.json`
...
- Sparkles in this candidate: **holes**. Envelope: **shadow-only**.
...
1. Confirm `pick.json` slug is `2026-08-23-after-tweaks`.
2. Resize `2026-08-23-after-tweaks.png` into `MailGent/Assets.xcassets/AppIcon.appiconset/` (same filenames, no extra keyline):
- icon_16x16.png (16px)
...
7. Menu-bar template files are **preview + pack only**. Live app still uses SF Symbol `tray.full` (`MenuBarIcon.swift`). Do not swap the status item unless the user asks.

## Do not

- Stamp, MCP preview, or compile a different candidate than `2026-08-23-after-tweaks`.
